...
public class TeacherController {
    ...
    public CommonResult<Object> getQuestionBank() {
        ...
        if (redisUtil.get("questionBanks") != null) {//redis中有缓存
            ...
            return new CommonResult<>(200, "success",
                    redisUtil.get("questionBanks"));
        } else {//redis无缓存
            List<QuestionBank> questionBanks = questionBankService.list(
                    new QueryWrapper<>());
            //设置默认缓存时间(10分钟) + 随机缓存时间(0-5分钟 )  来防止缓存雪崩和击穿
            redisUtil.set("questionBanks", questionBanks, 60 * 10 +
                    new Random().nextInt(5) * 60);
            return new CommonResult<>(200, "success", questionBanks);
        }
    }

    @GetMapping("/getQuestion")
    @ApiOperation("获取题目信息,可分页 ----> 查询条件(可无)" +
            "(questionType,questionBank,questionContent),必须有的(pageNo,pageSize)")
    public CommonResult<List<Question>> getQuestion(
        ...
        if (!Objects.equals(questionType, "")) wrapper.eq("qu_type",
                questionType);
        if (!Objects.equals(questionBank, "")) wrapper.like("qu_bank_name",
                questionBank);
        if (!Objects.equals(questionContent, "")) wrapper.like("qu_content",
                questionContent);
    ...
    public CommonResult<String> deleteQuestion(String questionIds)
        ...
        return flag ? new CommonResult<>(200, "删除成功") :
                new CommonResult<>(233, "删除失败");
    }
    ...
    public CommonResult<String> addBankQuestion(String questionIds, String banks) {
        ...
        return flag ? new CommonResult<>(200, "添加题库成功") :
                new CommonResult<>(233, "添加题库失败");
    }
    ...
    public CommonResult<String> removeBankQuestion(String questionIds, String banks) {
        ...
        return flag ? new CommonResult<>(200, "移除题库成功") :
                new CommonResult<>(233, "移除题库失败");
    }
    ...
    public CommonResult<String> operationExam(@PathVariable("type") Integer type, String ids) {
        ...
        if (type == 1) {
            for (String s : id) {
                Exam exam = examService.getOne(new QueryWrapper<Exam>().eq("exam_id",
                        Integer.parseInt(s)));
                exam.setStatus(1);
    ...
    public CommonResult<String> addExamByQuestionList
            (@RequestBody AddExamByQuestionVo addExamByQuestionVo) {
    ...
    public CommonResult<Integer> addExamRecord(@RequestBody ExamRecord examRecord,
        ...
        ExamQuestion examQuestion = examQuestionService.getOne(
                new QueryWrapper<ExamQuestion>().eq("exam_id",
    ...
    @ApiOperation("设置考试记录的主观题得分,设置总分为逻辑得分+客观题")
    public CommonResult<String> setObjectQuestionScore(Integer totalScore,
        ...
        return flag ? new CommonResult<>(200, "批阅成功") :
                new CommonResult<>(233, "批阅失败");
    }
    ...
    @ApiOperation("提供每一门考试的通过率数据(echarts绘图)")
        ...
        List<ExamRecord> examRecords = examRecordService.list(
                new QueryWrapper<ExamRecord>().isNotNull("total_score"));
        //考试的名称
